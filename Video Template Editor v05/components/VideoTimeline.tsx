...
import { ZoomIn, ZoomOut, Play, Pause, SkipBack, SkipForward, Film, Clock, Volume2, Type, Music, Undo, Redo, Plus } from 'lucide-react';
...
export function VideoTimeline({
  ...
  // Calculate playhead position based on timeline view
  const getPlayheadPosition = useCallback(() => {
    if (!currentVideo) return 0;
    const timelineViewDuration = getTimelineViewDuration();
    // Position playhead based on current time relative to start of clip
    const absoluteTime = currentVideo.position.startTime + currentTime;
    const progress = absoluteTime / timelineViewDuration;
    return Math.min(Math.max(progress * 100, 0), 100);
  }, [currentVideo, currentTime, getTimelineViewDuration]);

  // Handle timeline scrubbing with proper timeline view duration
  const handleTimelineClick = useCallback((e: React.MouseEvent) => {
    if (!currentVideo || !timelineRef.current || trimState?.isActive) return;

    const rect = timelineRef.current.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const progress = x / rect.width;
    const timelineViewDuration = getTimelineViewDuration();
    const absoluteTime = progress * timelineViewDuration;
  ...
  }, [currentVideo, onTimeUpdate, getTimelineViewDuration, trimState]);
  ...
  const generateTimelineMarkers = () => {
    ...
    // Calculate marker interval based on timeline duration and zoom level
    let interval = 5; // Default 5 second intervals
    let subInterval = 1; // Sub-markers
    
    if (timelineViewDuration <= 10) {
      interval = 1;
      subInterval = 0.25;
    } else if (timelineViewDuration <= 30) {
      interval = 2;
      subInterval = 0.5;
    } else if (timelineViewDuration <= 60) {
      interval = 5;
      subInterval = 1;
    } else if (timelineViewDuration <= 120) {
      interval = 10;
      subInterval = 2;
    } else {
      interval = 30;
    ...
    // Generate main markers for the entire timeline view
    for (let time = 0; time <= timelineViewDuration; time += interval) {
      const position = (time / timelineViewDuration) * 100;
    ...
    for (let time = subInterval; time <= timelineViewDuration; time += subInterval) {
  ...
  // Calculate clip dimensions accurately based on timeline view and original duration
  const getClipDimensions = () => {
    if (!currentVideo || !timelineRef.current) {
    ...
    // Calculate clip width based on trimmed duration within the timeline view
    const clipWidthPercentage = (trimmedDuration / timelineViewDuration) * 100;
    const clipWidth = Math.max(80, (availableWidth * clipWidthPercentage) / 100);
    
    // Calculate clip left position based on start time within the timeline view
    const clipLeftPercentage = (currentVideo.position.startTime / timelineViewDuration) * 100;
    const clipLeft = (availableWidth * clipLeftPercentage) / 100;
  ...
  // Trim handle mouse event handlers
  ...
  const handleTrimMouseMove = useCallback((e: MouseEvent) => {
    if (!trimState?.isActive || !currentVideo || !timelineRef.current || !onVideoUpdate) return;

    const rect = timelineRef.current.getBoundingClientRect();
    const timelineViewDuration = getTimelineViewDuration();
    const deltaX = e.clientX - trimState.startX;
    const deltaTime = (deltaX / rect.width) * timelineViewDuration;
    
    // Get original full duration
    const originalFullDuration = getOriginalDuration();
    ...
    if (trimState.side === 'left') {
      ...
      newEndTime = Math.min(
        originalFullDuration, // Can't exceed original duration
        Math.max(
  ...
  }, [trimState, currentVideo, onVideoUpdate, getTimelineViewDuration, getOriginalDuration]);
  ...
  // Add mouse event listeners for trimming
  React.useEffect(() => {
    if (trimState?.isActive) {
  ...
  const renderVideoClip = () => {
    ...
    return (
      ...
      >
        ...
        {/* Left trim handle */}
        <div
          className="absolute left-0 top-0 bottom-0 w-2 bg-primary/20 hover:bg-primary/40 cursor-ew-resize opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"
        ...
        {/* Right trim handle */}
        <div
          className="absolute right-0 top-0 bottom-0 w-2 bg-primary/20 hover:bg-primary/40 cursor-ew-resize opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"
        ...
        <div className="absolute inset-0 flex items-center justify-between px-3 pointer-events-none">
          ...
          <div className="flex items-center space-x-2 min-w-0">
            <div className="bg-black/70 rounded-full p-1.5">
              <Film className="w-4 h-4 text-white flex-shrink-0" />
            </div>
            <span className="text-sm text-white truncate font-medium" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.8)' }}>
              {currentVideo.name}
            </span>
          </div>
    ...
    <div className="h-full flex flex-col bg-card">
      ...
      <div className="border-b border-border">
        ...
        <div className="p-3 flex items-center justify-between">
          ...
          {/* Center - Track type indicators */}
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-1 text-xs text-muted-foreground">
              <Film className="w-3 h-3" />
              <span>Video</span>
            </div>
            <div className="flex items-center space-x-1 text-xs text-muted-foreground opacity-50">
      ...
      {/* Timeline Content Area - Full Height */}
      <div className="flex-1 relative overflow-hidden">
        ...
        <div
          ref={timelineRef}
          className="relative h-full w-full cursor-pointer bg-muted/10"
        ...
        >
          ...
          {/* Playhead - Positioned based on timeline view */}
          {currentVideo && (
            <div
